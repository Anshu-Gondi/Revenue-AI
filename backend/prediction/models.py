...
def train_model_pipeline(df: pl.DataFrame, model_name='random_forest'):
    ...
    for col in cat_cols:
        le = LabelEncoder()
        encoded = le.fit_transform(df[col].to_pandas().fillna("MISSING"))
    ...
    if "month" in X.columns:
        ...
        for i in range(1, 6):
            ...
            rows.append(row)
        future_months = pl.DataFrame(rows).to_pandas()[X.columns]
